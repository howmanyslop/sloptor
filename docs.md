...
- **Native declaration emit, with `import("alias/...")` types resolved** — rotor emits every `.d.ts` from the native compiler instead of routing declaration emit through the Node worker, and ports rbxtsc's two `afterDeclarations` transformers (the `types` triple-slash rescope and the `paths`/`baseUrl` -> relative-specifier rewrite) to Go. The rewrite is a resolution result driven by a reparse of the emitted text, never a regex, and the `.d.ts.map` generated columns are shifted to follow each spliced specifier. One **deliberate divergence** from rbxtsc comes with it: the JS transformer only ever resolved specifiers the program had already resolved for that file, so an `import("alias/...")` type the declaration emitter SYNTHESIZED for an inferred type kept its alias spelling and shipped unresolvable; rotor falls back to a real resolver pass for those, so they are rewritten too. **Not supported:** an `afterDeclarations` transformer of your own — the native compiler has no custom-transformer hook, so a project that declares one gets a clear per-build warning rather than a silent drop (no plugin in the `@rbxts` ecosystem ships one).
...
`sloptor build` accepts the rbxtsc-compatible flag surface (booleans accept `--flag`, `--flag=false`, `--no-flag`): `-p/--project`, `-b/--build [path]`, `--builders <n>`, `--checkers <n>`, `--singleThreaded`, `--emitDeclarationOnly`, `-w/--watch`, `--usePolling`, `--verbose`, `--noInclude`, `--logTruthyChanges`, `--writeOnlyChanged`, `--writeTransformedFiles` (parsed and ignored), `--optimizedLoops`, `--type game|model|package`, `-i/--includePath`, `--rojo`, `--allowCommentDirectives`, and `--luau`. Sloptor adds `--cpuprofile`, `--trace-out`, `--blockprofile`, `--mutexprofile`, `--heapprofile`, `--timings`, `--minify`, `--max-errors`, `--json`, `--bell`, and `--no-clear`. `--builders` requires `--build`. `--singleThreaded` is the tsgo flag of the same name: when effective it forces one solution builder and one checker per program, even if `--builders`/`--checkers` are larger. There is no `--parallel` flag. `--emitDeclarationOnly` requires `--build`; `--build --watch` cannot be combined with declaration-only emit. The finite profiling and timing flags cannot be combined with `--watch`. Run `sloptor build --help` for the rendered descriptions.
...
- **`--json`** — emit one machine-readable result object (version, ok, files, durationMs, diagnostics with `file`/`line`/`col`/`code`/`severity`/`message`) instead of styled output. `file` is relative to the project directory (the solution tsconfig directory with `--build`). `code` is `TS####` for a TypeScript diagnostic and the upstream factory name for a transformer one, and is omitted when the diagnostic has none. Also available on `sloptor check`.
...
`sloptor build --build` drains project references in dependency order. A coordinator tsconfig with only `references` is not compiled as a project of its own. Each referenced project gets the options its own direct build would: defaults, then its `rbxts` key, then the command line. The entry project's `rbxts` key does not reach its references. A coordinator's `rbxts` key applies to every project, below each project's own key. `type` and `rojo` are the exception: `--type`, `--rojo` and the coordinator's values reach only projects that have no `rbxts` key, and only under a coordinator. `--emitDeclarationOnly` applies to the solution build and cannot be watched. `--build --watch` watches the solution's tsconfig extends chain and Rojo topology, invalidating dependent projects when a referenced project changes.
...
On an incremental rebuild Rotor narrows the sidecar's `LanguageService` root set to the files it is recompiling plus every file that can reach them without being imported — declaration files, global scripts, and modules that open `declare global` — instead of handing the worker the whole project. TypeScript pulls each root's import closure into the program, so a transformer that asks the checker about a file it was handed gets the same answers. **A transformer that instead enumerates `program.getSourceFiles()` to build program-wide state sees only the narrowed program on an incremental rebuild, and can go stale.** `rbxts-transformer-jecs`, `rbxts-transformer-jest`, the React-compiler applier and native Flamework are all per-file and unaffected. A full build is never narrowed, so deleting the project's `*.rbxtsc.tsbuildinfo` is the escape hatch for a transformer that needs the whole program.
...
A resolved `.json` module is hashed into the manifest as an input even though nothing compiles it: its value never reaches an importer's Luau (that becomes a runtime require of the copied file), but its inferred type does reach the importer's `.d.ts`, so a change to it rebuilds whoever imported it.
