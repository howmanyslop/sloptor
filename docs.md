...
The declaration comparison only runs where Rotor writes the declaration itself. A project whose declarations are rewritten by the Node sidecar — one with external transformer plugins, or with `paths`/`baseUrl` — and a native Flamework project, whose declarations are emitted from the transformed program, keep the plain importer-closure selection until declaration emit moves to Rotor's own emitter for every project.
