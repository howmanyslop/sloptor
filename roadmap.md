...
- [x] **Loop label support (August 6, 2026 — rotor extension, second deliberate superset of rbxtsc after `$getModuleTree` folders)** — rbxtsc 3.0.0 bans every labeled statement (`noLabeledStatement`); rotor compiles `outer: for (...) { break outer; continue outer; }`. Luau has neither labels nor `goto`, so a label that a plain `break`/`continue` cannot satisfy lowers to a `"none" | "break" | "continue"` flag temp plus unwinding checks after each enclosing break boundary (`internal/transformer/labeledstatement.go` + `State.LabelChecks`/`LabelResets`). Design follows roblox-ts PR [#2928](https://github.com/roblox-ts/roblox-ts/pull/2928) but fixes five defects in it locally: the depth/index heuristic that breaks an *unlabeled* enclosing loop, `switch` not counted as a break boundary (rotor lowers it to `repeat ... until true`, which a bare `break` exits), labeled non-loop statements emitting a bare `break`, no function/`try` barrier (a check inside a closure would break the wrong loop), and only the first of two labels on one loop getting a continue check. Rejected with clear diagnostics rather than wrong output: labels crossing `try`/`catch` (`noLabeledStatementsWithinTryCatch`, message byte-exact with the PR), unknown labels behind `// @ts-ignore` (`rotorUnknownLabel`), a labeled non-loop statement whose body also holds an unlabeled `break`/`continue` bound to an enclosing loop (`rotorLabeledStatementFlowControl`), and a labeled `continue` targeting a `do`/`while` whose condition needs temporaries (`rotorLabeledContinueInDoWhile` — Luau forbids `continue` before those locals; the identical *unlabeled* hole is an rbxtsc bug rotor reproduces for parity). Byte parity is untouched: no label program compiles under rbxtsc, so `testdata/diff` and `testdata/conformance` goldens are unchanged. Pinned by `internal/transformer/labels_test.go` (authored expectations, not rbxtsc-derived), the rotor-owned Lune spec `testdata/conformance/rotor/label.spec.ts` (21 runtime cases), and the end-to-end fixture `internal/compile/testdata/labels_model`
